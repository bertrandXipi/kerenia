...
import { Star, ChevronRight, Droplets, Mountain, Home as HomeIcon, Waves, Calendar, Users, Search, ArrowDown } from 'lucide-react';
import { REVIEWS, APARTMENTS, BOOKING_URL } from '../constants';
...
const Home: React.FC = () => {
  ...
  const scale = useTransform(scrollY, [0, 1000], [1.1, 1]); // Ken Burns start zoom
  ...
  const handleSearch = (e: React.MouseEvent) => {
    ...
    const finalUrl = `${BOOKING_URL}${separator}${params.toString()}`;
  ...
  return (
    <>
      ...
      {/* overflow-visible needed for the booking bar to overhang */}
      <section ref={targetRef} className="relative h-screen flex flex-col justify-center items-center pt-20">
        
        {/* Background Image Container with Overflow Hidden to clip parallax image */}
        ...
        <div className="relative z-10 container mx-auto px-6 text-center mt-8 md:mt-0">
          <motion.div style={{ opacity }}>
             {/* Animated Title Box - Reveal Effect */}
            ...
            >
                <p className="font-script text-4xl md:text-6xl mb-6 text-white font-light text-shadow-sm">
                L’expérience bien-être,
                </p>
                <div className="flex items-center justify-center gap-6">
                     <div className="h-px w-12 bg-white/80"></div>
                     <p className="font-sans font-light text-xs md:text-sm tracking-[0.3em] uppercase text-white/90 shadow-black drop-shadow-md">
        ...
        {/* Floating Booking Bar - Straddling the bottom edge */}
        <motion.div 
        ...
        >
            <div className="bg-white/95 backdrop-blur-xl rounded-[2rem] shadow-2xl p-2 md:p-3 flex flex-col md:flex-row items-center justify-between gap-2 md:gap-4 border border-white/60 relative ring-1 ring-black/5">
                ...
                <button 
                    ...
                    className="bg-brick-700 hover:bg-brick-800 text-white rounded-full px-8 py-4 flex items-center gap-3 transition-all shadow-lg hover:shadow-brick-700/30 transform hover:scale-[1.02] w-full md:w-auto justify-center group"
                >
                    <Search size={18} className="group-hover:scale-110 transition-transform" />
                    <span className="font-condensed font-bold uppercase tracking-[0.15em] text-xs whitespace-nowrap">Disponibilités</span>
                </button>
            </div>
        </motion.div>
      </section>

      {/* Intro Section - increased top padding to account for floating booking bar */}
        ...
        <div className="container mx-auto px-6">
          <div className="text-center mb-16">
            ...
            <p className="text-slate-600 max-w-2xl mx-auto font-light text-lg">
                Notre résidence dispose de 16 appartements de 25m2 à 45m2 avec terrasses, entièrement rénovés, une décoration soignée dans une atmosphère chaleureuse.
            </p>
          </div>
            ...
            {APARTMENTS.map((apt, index) => (
              ...
              >
                ...
                <div className="p-8 text-center">
                  ...
                  <Link to="/appartements" className="inline-block relative overflow-hidden group/btn px-8 py-3 border border-brick-700 text-brick-800 font-condensed font-bold text-xs uppercase tracking-[0.2em]">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between mb-12">
            <div className="mb-6 md:mb-0">
               ...
               <div className="flex items-center gap-2 mt-3">
                 ...
                 <img src="https://upload.wikimedia.org/wikipedia/commons/2/2f/Google_2015_logo.svg" alt="Google" className="h-5 ml-2 opacity-60 grayscale hover:grayscale-0 transition-all"/>
               </div>
            </div>
            <a href="https://www.google.com/maps" target="_blank" rel="noreferrer" className="text-brick-600 font-condensed font-bold uppercase text-sm tracking-widest border-b border-brick-600 hover:text-brick-800 transition-colors pb-1">
                Voir tous les avis
            </a>
          </div>
          ...
          <div className="relative h-[450px] group overflow-hidden cursor-pointer">
            ...
            <div className="absolute inset-0 bg-black/30 group-hover:bg-black/50 transition-colors duration-500 flex flex-col items-center justify-center text-center p-8">
                <motion.div initial={{ y: 20, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }}>
                    <h3 className="text-white font-condensed font-light text-4xl uppercase mb-4 shadow-black drop-shadow-md tracking-[0.2em]">Piscine</h3>
                    <p className="text-white mb-8 max-w-sm font-light drop-shadow-sm opacity-0 group-hover:opacity-100 transition-opacity duration-500 transform translate-y-4 group-hover:translate-y-0">Venez vous détendre autour de notre piscine chauffée avec des bains de soleil.</p>
                    <Link to="/appartements" className="bg-white text-brick-900 px-8 py-3 font-condensed font-bold uppercase tracking-[0.2em] text-xs hover:bg-brick-700 hover:text-white transition-colors shadow-lg">
                        RESERVER
                    </Link>
                </motion.div>
            </div>
          ...
          <div className="relative h-[450px] group overflow-hidden cursor-pointer">
            ...
            <div className="absolute inset-0 bg-black/30 group-hover:bg-black/50 transition-colors duration-500 flex flex-col items-center justify-center text-center p-8">
                <motion.div initial={{ y: 20, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} transition={{ delay: 0.3 }}>
                    <h3 className="text-white font-condensed font-light text-4xl uppercase mb-4 shadow-black drop-shadow-md tracking-[0.2em]">Autour de nous</h3>
                    <p className="text-white mb-8 max-w-sm font-light drop-shadow-sm opacity-0 group-hover:opacity-100 transition-opacity duration-500 transform translate-y-4 group-hover:translate-y-0">Le Pays basque s’offre à vous. Retrouvez ici les coups de cœur de Ker Enia.</p>
                    <Link to="/autour-de-nous" className="bg-transparent border border-white text-white px-8 py-3 font-condensed font-bold uppercase tracking-[0.2em] text-xs hover:bg-white hover:text-brick-800 transition-colors backdrop-blur-sm">
                        DÉCOUVRIR
                    </Link>
                </motion.div>
            </div>

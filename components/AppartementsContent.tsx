        ...
        {APARTMENTS.map((apt, index) => (
          ...
          >
            ...
            <div className="w-full md:w-1/2 space-y-6">
              <h2 className="font-script text-4xl md:text-5xl text-brick-600">{apt.title}</h2>
              <div className="flex gap-4 text-sm font-bold text-brick-600 uppercase tracking-wider border-b border-brick-200 pb-4">
                <span>{apt.size}</span>
              ...
              <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                {apt.features.map((feature, i) => (
                  <li key={i} className="flex items-center gap-2 text-slate-700 text-sm">
                    <Check size={14} className="text-brick-600" />
                    {feature}
                  </li>
                ))}
              </ul>
